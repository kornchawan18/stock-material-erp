...
import { getMaterials, issueStock } from '../api/api'
...
export default function Issue() {
  ...
  const [docNo, setDocNo] = useState('GI-' + new Date().toISOString().slice(0,10) + '-001')
  ...
  const setQty = (id, qty, max) => setCart(c => ({ ...c, [id]: { ...c[id], qty: Math.max(1, Math.min(max, qty)) } }))
  ...
  async function submit() {
    ...
    setLoading(true)
    try {
      await issueStock({ items: cartItems.map(x => ({ mat_id: x.mat.id, qty: x.qty })), doc_no: docNo, date, note, dept })
      toast(`บันทึกเบิกออก ${cartItems.length} รายการ สำเร็จ`, 'success')
      setCart({}); setDocNo('GI-' + new Date().toISOString().slice(0,10) + '-' + String(Date.now()).slice(-3)); setNote(''); setDept('')
      load()
  ...
  return (
    <>
      <div className="page-header">
        <div><div className="page-title">⬇ เบิกวัสดุออก (Goods Issue)</div><div className="page-subtitle">บันทึกการเบิกวัสดุออกจากคลัง</div></div>
      </div>
      <div className="gr-layout">
              ...
              <table>
                ...
                <tbody>
                  {rows.map(m => {
                    ...
                    const qColor = m.qty === 0 ? 'var(--danger)' : 'inherit'
                    return (
                      <tr key={m.id} style={inCart?{background:'#fffbeb'}:{}}>
                        <td><button className={`btn btn-sm ${inCart?'btn-warning':'btn-outline'}`} onClick={()=>toggle(m)}>{inCart?'✓':'+'}</button></td>
                        ...
                        <td style={{textAlign:'center',fontWeight:700,color:qColor}}>{m.qty?.toLocaleString()}</td>
                      </tr>
                    )
                  })}
                </tbody>
        ...
        <div className="card sticky-card">
          <div className="card-header"><span className="card-title">📋 รายการเบิกออก ({cartItems.length})</span></div>
          <div className="card-body">
            {cartItems.length === 0
                  ...
                  {cartItems.map(({ mat, qty }) => (
                    <div key={mat.id} className="cart-item">
                      <div className="cart-item-info">
                        ...
                        <div className="cart-stock">Stock: {mat.qty}</div>
                      </div>
                      <input className="qty-input" type="number" min="1" max={mat.qty} value={qty} onChange={e=>setQty(mat.id,+e.target.value,mat.qty)} />
            ...
            <div className="form-group" style={{marginBottom:10}}>
              <label className="form-label">วันที่เบิก</label>
              <input className="form-control" type="date" value={date} onChange={e=>setDate(e.target.value)} />
            </div>
            <div className="form-group" style={{marginBottom:10}}>
              <label className="form-label">หน่วยงาน/โครงการ</label>
              <input className="form-control" value={dept} onChange={e=>setDept(e.target.value)} placeholder="ระบุหน่วยงาน..." />
            </div>
            <div className="form-group" style={{marginBottom:16}}>
              <label className="form-label">หมายเหตุ</label>
              <textarea className="form-control" rows="2" value={note} onChange={e=>setNote(e.target.value)} />
            </div>
            <button className="btn btn-warning" style={{width:'100%'}} onClick={submit} disabled={loading}>
              {loading ? '...' : '✓ บันทึกเบิกออก'}
            </button>
          </div>
        </div>
      </div>
